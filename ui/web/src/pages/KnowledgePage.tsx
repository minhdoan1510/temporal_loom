...
export default function KnowledgePage() {
  ...
  // Auto-poll while any KB is syncing
  useEffect(() => {
  ...
  const openEdit = (kb: KnowledgeBase, e: React.MouseEvent) => {
    ...
    setForm({
      ...
      space_key: kb.space_key,
      root_page: kb.root_page,
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await knowledge.update(editingId, form);
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Manage knowledge bases indexed from Confluence into Qdrant
          </p>
        </div>
        {canCreate && (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="flex max-h-[85vh] sm:max-w-lg flex-col border-border/50 bg-sidebar">
          ...
          <div className="flex-1 space-y-4 overflow-y-auto pr-1">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="e.g. cash_loan_knowledge_base"
                className="bg-card"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <Input
                value={form.root_page}
                onChange={(e) => setForm({ ...form, root_page: e.target.value })}
                placeholder="e.g. [PCF-FS][Cash Loan] Knowledge base"
                className="bg-card"
              />
            </div>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={handleSave}
              disabled={saving || !form.name || !form.collection || !form.space_key || !form.root_page}
              className="cursor-pointer"
            >
              {saving ? "Saving..." : "Save"}
            </Button>
